...
TEST_CASE("data round trips through a dataset") {
        ...
        arf::h5d::dataset d = entry.create_dataset("pcm", written);
...
TEST_CASE("the vector read resizes to the dataset") {
        ...
        file f(scratch.path, "w");
        group entry(f, "entry", true);
        arf::h5d::dataset d = entry.create_dataset("pcm", arftest::ramp(37));
...
TEST_CASE("writing resizes the dataset to match") {
        ...
        file f(scratch.path, "w");
        group entry(f, "entry", true);
        arf::h5d::dataset d = entry.create_dataset("pcm", arftest::ramp(100));
...
TEST_CASE("set_extent grows and shrinks") {
        ...
        file f(scratch.path, "w");
        group entry(f, "entry", true);
        arf::h5d::dataset d = entry.create_dataset("pcm", arftest::ramp(50));
...
TEST_CASE("datasets are chunked, and report their chunk shape") {
        ...
        file f(scratch.path, "w");
        group entry(f, "entry", true);
        arf::h5d::dataset d = entry.create_dataset("pcm", arftest::ramp(1 << 20));
...
TEST_CASE("every dataset is filtered by default, at deflate level 0") {
        ...
        arf::h5d::dataset defaulted = entry.create_dataset("defaulted", data);
        CHECK(filter_count(defaulted) == 1);

        arf::h5d::dataset framed = entry.create_dataset("framed", data, 0);
        CHECK(filter_count(framed) == 1);

        arf::h5d::dataset squeezed = entry.create_dataset("squeezed", data, 9);
        CHECK(filter_count(squeezed) == 1);

        // opting out has to be explicit
        arf::h5d::dataset raw = entry.create_dataset("raw", data, -1);
...
TEST_CASE("deflate level 0 frames the data without compressing it") {
        ...
        arf::h5d::dataset unfiltered = entry.create_dataset("unfiltered", flat, -1);
        arf::h5d::dataset framed = entry.create_dataset("framed", flat, 0);
        arf::h5d::dataset squeezed = entry.create_dataset("squeezed", flat, 9);
        f.flush();
...
TEST_CASE("compression shrinks a compressible dataset") {
        ...
        arf::h5d::dataset squeezed = entry.create_dataset("squeezed", flat, 9);
        arf::h5d::dataset raw = entry.create_dataset("raw", flat, -1);
        f.flush();
...
TEST_CASE("the storage type can differ from the memory type") {
        ...
        arf::h5d::dataset d = entry.create_dataset<double, float>("pcm", written);
...
TEST_CASE("an empty dataset can be created and grown") {
        ...
        arf::h5d::dataset d = entry.create_dataset("empty", nothing);
...
TEST_CASE("introspection releases the handles it opens") {
        ...
        file f(scratch.path, "w");
        group entry(f, "entry", true);
        arf::h5d::dataset d = entry.create_dataset("pcm", arftest::ramp(8));
...
TEST_CASE("datasets expose their dataspace and datatype") {
        ...
        file f(scratch.path, "w");
        group entry(f, "entry", true);
        arf::h5d::dataset d = entry.create_dataset("pcm", arftest::ramp(12));
